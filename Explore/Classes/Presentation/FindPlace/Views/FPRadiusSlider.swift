...
//  Explore
//
//  Created by Andrey Chernyshev on 12.09.2020.
//  Copyright © 2020 Andrey Chernyshev. All rights reserved.
//

import UIKit

@IBDesignable open class FPRadiusSlider: UIControl {
    ...
    public convenience init(frame: CGRect = .zero, completion: ((FPRadiusSlider) -> Void)? = nil) {
        self.init(frame: frame)
        completion?(self)
    }

    var didChangedValue: ((Int) -> Void)?
    ...
    fileprivate func refresh() {
        ...
        // update the delegate
        if handleTracking != .none {
            didChangedValue?(Int(selectedMaxValue))
        }
    }
...
private final class RangeSeekSliderLeftElement: UIAccessibilityElement {
    ...
    override func accessibilityIncrement() {
        guard let slider = accessibilityContainer as? FPRadiusSlider else { return }
